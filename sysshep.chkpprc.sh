...
# autogenerated Thu Jan 05 16:27:05 MST 2017 by taylor using command:
# gensh -to sysshep.chkpprc.sh -path /opt/ublu/ublu.jar -includepath $SCRIPTDIR -optr p PROPERTIES @properties ${ credentials properties file }$ -optr q ENV @env ${ env to check }$ -optr q TYPE @type ${ type to check }$ ${ sysshep.chkpprc.sh: run chkpprc and output an info or critical datapoint on the result }$ sysshep/chkpprc.ublu ${ sysshep.chkpprc ( @properties @env @type ) }$

# Usage message
function usage {
echo "sysshep.chkpprc.sh: run chkpprc and output an info or critical datapoint on the result "
echo "This shell script was autogenerated Thu Jan 05 16:27:05 MST 2017 by taylor."
echo "Usage: $0 [silent] -h -p PROPERTIES -q ENV -q TYPE "
echo "	where"
echo "	-h		display this help message and exit 0"
echo "	-p PROPERTIES	credentials properties file  (required option)"
echo "	-q ENV	env to check  (required option)"
echo "	-q TYPE	type to check  (required option)"
echo "---"
...
then
	...
	echo "Option -q ENV is a required option but is not present."
	usage
	exit 2
fi
if [ "${TYPE}" != "" ]
then
	...
	echo "Option -q TYPE is a required option but is not present."
	usage
	exit 2
fi
